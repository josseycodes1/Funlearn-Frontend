...
export default function ChatBotPage() {
  ...
  const { setShowGeneralSidebar, setSidebarCollapsed, showGeneralSidebar, sidebarCollapsed } = useSidebar();
  ...
  return (
    <div className="flex h-screen bg-gray-50 fixed inset-0">
      {/* Main Dashboard Sidebar - FIXED: Lower z-index */}
      <DashboardSidebar
        activeTab={activeTab}
      ...
      <div className="flex-1 flex flex-col min-h-0 relative">
        {/* Dashboard Header - Now with proper z-index */}
        ...
        <div className="flex-1 flex bg-white min-h-0">
          {/* Chat History Sidebar - Fixed z-index */}
          <div
            className={`
              w-80 bg-gray-50 border-r border-gray-200 flex flex-col transition-all duration-300
              lg:translate-x-0 lg:relative lg:z-10
              ${showChatSidebar ? "translate-x-0 fixed z-30 inset-y-0 left-0 h-full" : "-translate-x-full fixed h-full"}
            `}
          >
            {/* Header */}
          ...
          {showChatSidebar && (
            <div
              className="fixed inset-0 bg-gray-600 bg-opacity-30 z-20 lg:hidden"
              onClick={() => setShowChatSidebar(false)}
            />
          )}
